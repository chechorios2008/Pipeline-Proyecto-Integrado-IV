...
-- (en inglés), el número de pedidos y sus ingresos totales. 
-- La primera columna será
-- Category, que contendrá las 10 categorías con menores ingresos; 
-- la segunda será
-- Num_order, con el total de pedidos de cada categoría; 
-- la última será Revenue,
...
INNER JOIN product_category_name_translation cnt ON prod.product_category_name = cnt.product_category_name
WHERE ord.order_status = 'delivered'
AND ord.order_delivered_customer_date IS NOT NULL
GROUP BY prod.product_category_name
ORDER BY SUM(pay.payment_value) ASC
LIMIT 10;